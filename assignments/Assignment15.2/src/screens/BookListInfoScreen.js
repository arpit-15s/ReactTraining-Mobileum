...
class BookListInfoScreen extends React.Component {
    ...
    state = {
      isbn: this.books[0].isbn
    }

    render(){

      const setIsbn = (temp_isbn) => {
        // console.log(temp_isbn)
        this.setState({isbn:temp_isbn});
      } 
  
      const searchByIsbn = (isbn) => {
        // console.log(isbn);
        const selected_book = this.books.find(book => book.isbn === this.state.isbn);
          ...
          <div className='BookListInfoScreen'>
              ...
              <div className='row'>
                  <div className='col col-3'>
                      <BookListComponent books={this.books} setIsbn={setIsbn} />
                  </div>
                  <div className='col col-9'>
                      <BookDetailsComponent book={searchByIsbn(this.state.isbn)} />
                  </div>
              </div>
          </div>
      );
